...
UINT8 __fastcall m92ReadPort(UINT32 port)
{
	...
	{
	case 0x00:
		return ~DrvInput[0]; // player 1
	case 0x01:
		return ~DrvInput[1]; // player 2
	case 0x02:
		return (~DrvInput[4] & 0x7F) | m92_sprite_buffer_busy;
	case 0x03:
		return DrvInput[7]; // dip 3
	case 0x04:
		return DrvInput[5]; // dip 1
	case 0x05:
		return DrvInput[6]; // dip 2
	case 0x06:
		return ~DrvInput[2]; // player 3
	case 0x07:
		return ~DrvInput[3]; // player 4

	case 0x08:
		VezSetIRQLineAndVector(0, (m92_irq_vectorbase + 12) / 4, CPU_IRQSTATUS_NONE);
		return sound_status[0];
	case 0x09:
		VezSetIRQLineAndVector(0, (m92_irq_vectorbase + 12) / 4, CPU_IRQSTATUS_NONE);
		return sound_status[1];

	case 0x18:
		return (m92_kludge == 3) ? MSM6295Read(0) : 0; // ppan
...
static INT32 MemIndex(INT32 gfxlen1, INT32 gfxlen2)
{
	...
	m92_layers[0] = (struct _m92_layer *)Next;
	Next += sizeof(struct _m92_layer);
	m92_layers[1] = (struct _m92_layer *)Next;
	Next += sizeof(struct _m92_layer);
	m92_layers[2] = (struct _m92_layer *)Next;
	Next += sizeof(struct _m92_layer);

	DrvPalette = (UINT32 *)Next;
	Next += 0x0801 * sizeof(UINT32);
...
{
	if (BurnLoadRom(DrvV33ROM + 0x000001, 0, 2))
		return 1;
	if (BurnLoadRom(DrvV33ROM + 0x000000, 1, 2))
		return 1;
	if (BurnLoadRom(DrvV33ROM + 0x000001 + v33off, 2, 2))
		return 1;
	if (BurnLoadRom(DrvV33ROM + 0x000000 + v33off, 3, 2))
		return 1;

	if (BurnLoadRom(DrvV30ROM + 0x000001, 4, 2))
		return 1;
	if (BurnLoadRom(DrvV30ROM + 0x000000, 5, 2))
		return 1;
	...
	if (gfxtype1)
	{
		...
		if (BurnLoadRom(DrvSndROM + 0x000000, 18, 1))
			return 1;
	}
	else
	{
		...
		if (BurnLoadRom(DrvSndROM + 0x000000, 14, 1))
			return 1;
	}

	if (eep)
	{
		if (BurnLoadRom(DrvEEPROM + 0x000000, eep, 1))
			return 1;
	}
...
{
	if (sx <= (0 - width) || sx >= nScreenWidth || sy <= (0 - height) || sy >= nScreenHeight)
		return;
		...
		for (INT32 offs = 0; offs < m92_sprite_list;)
		{
			...
			for (INT32 j = 0; j < x_multi; j++)
			{
				...
				for (INT32 i = 0; i < y_multi; i++)
				{
					...
					if (x > 0x1f0)
						RenderTilePrio(pTransDraw, DrvGfxROM1, (code + s_ptr) & graphics_mask[1], color << 4, x - 512, y - i * 16, flipx, flipy, 16, 16, RamPrioBitmap, pri_b);
...
static void draw_layer_byline(INT32 start, INT32 finish, INT32 layer, INT32 forcelayer)
{
	...
	for (INT32 sy = start; sy < finish; sy++)
	{
		...
		if (ptr->enable_rowscroll)
			scrollx_1 += BURN_ENDIAN_SWAP_INT16(ptr->scroll[(sy + scrolly) & 0x1ff]) - ((m92_kludge == 4) ? 24 : 0);
...
static void DrawLayers(INT32 start, INT32 finish)
{
	...
	{
		if (nBurnLayer & 1)
			draw_layer_byline(start, finish, 2, 1);
		if (nBurnLayer & 1)
			draw_layer_byline(start, finish, 2, 0);
	}
	else if (!bTransDrawReseted)
	{
		memset(pTransDraw + (start * nScreenWidth), 0, nScreenWidth * (finish - start) * sizeof(INT16));
	}

	if (nBurnLayer & 2)
		draw_layer_byline(start, finish, 1, 1);
	if (nBurnLayer & 2)
		draw_layer_byline(start, finish, 1, 0);
	if (nBurnLayer & 4)
		draw_layer_byline(start, finish, 0, 1);
	if (nBurnLayer & 8)
		draw_layer_byline(start, finish, 0, 0);
}
...
static INT32 DrvDraw()
{
	...
	if (m92_ok_to_blank && m92_video_reg & 0x80)
		BurnTransferClear(0x800); // most-likely probably screen disable (fixes bad fades in nbbatman, rtypeleo)
...
static void scanline_interrupts(INT32 prev, INT32 segment, INT32 scanline)
{
	if (m92_sprite_buffer_timer)
	{
		// memcpy(DrvSprBuf, DrvSprRAM, 0x800);
		if (~m92_kludge & 4)
			nCyclesDone[0] += VezRun(347); // nbbatman: fix for random lockups during gameplay
		m92_sprite_buffer_busy = 0x80;
...
static INT32 ppanRomLoad()
{
	...
	if (BurnLoadRom(DrvSndROM + 0x100000, 14, 1))
		return 1;
