...
static INT32 MemIndex()
{
	...
	RamSprBak   = (UINT32 *) Next; Next += 0x0020000 * sizeof(UINT32);
...
static void cps3_draw_tilemapsprite_line(INT32 drawline, UINT32 * regs )
{
...

	{
		...
		if (!linescroll_enable)	{
			...
			scrollx =  (regs[0]&0xffff0000)>>16;
			scrollx+= (RamSprBak[linebase+((line+16-4)&0x3ff)]>>16)&0x3ff;
		}
		...
		for (INT32 x=0;x<(cps3_gfx_max_x/16)+2;x++) {
			...
			dat = RamSprBak[mapbase+((tileline&63)*64)+((x+scrollx/16)&63)];
...
INT32 DrvCps3Draw()
{
	...
	// Draw Sprites
	{
		for (INT32 i=0x00000/4;i<0x2000/4;i+=4) {
			INT32 xpos		= (RamSprBak[i+1]&0x03ff0000)>>16;
			INT32 ypos		= (RamSprBak[i+1]&0x000003ff)>>0;

			INT32 gscroll		= (RamSprBak[i+0]&0x70000000)>>28;
			INT32 length		= (RamSprBak[i+0]&0x01ff0000)>>14; // how many entries in the sprite table
			UINT32 start		= (RamSprBak[i+0]&0x00007ff0)>>4;

			INT32 whichbpp		= (RamSprBak[i+2]&0x40000000)>>30; // not 100% sure if this is right, jojo title / characters
			INT32 whichpal		= (RamSprBak[i+2]&0x20000000)>>29;
			INT32 global_xflip	= (RamSprBak[i+2]&0x10000000)>>28;
			INT32 global_yflip	= (RamSprBak[i+2]&0x08000000)>>27;
			INT32 global_alpha	= (RamSprBak[i+2]&0x04000000)>>26; // alpha / shadow? set on sfiii2 shadows, and big black image in jojo intro
			INT32 global_bpp	= (RamSprBak[i+2]&0x02000000)>>25;
			INT32 global_pal	= (RamSprBak[i+2]&0x01ff0000)>>16;
			...
			if ((RamSprBak[i+0]&0xf0000000) == 0x80000000) break;	
		
			for (INT32 j=0; j<length; j+=4) {
				
				UINT32 value1 = (RamSprBak[start+j+0]);
				UINT32 value2 = (RamSprBak[start+j+1]);
				UINT32 value3 = (RamSprBak[start+j+2]);
