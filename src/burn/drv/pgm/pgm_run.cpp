...
static INT32 pgmMemIndex()
{
	...
	if (BurnDrvGetHardwareCode() & HARDWARE_IGS_USE_ARM_CPU)
	{
		PGMARMShareRAM = Next;
		Next += 0x0010000;
		PGMARMShareRAM2 = Next;
		Next += 0x0010000;
		PGMARMRAM0 = Next;
		Next += 0x0001000; // minimum map is 0x1000 - should be 0x400
		PGMARMRAM1 = Next;
		Next += 0x0040000;
		PGMARMRAM2 = Next;
		Next += 0x0001000; // minimum map is 0x1000 - should be 0x400
	...
	PGMBgRAM = (UINT32 *)Next;
	Next += 0x0001000;
	PGMTxtRAM = (UINT32 *)Next;
	Next += 0x0002000;

	PGMRowRAM = (UINT16 *)Next;
	Next += 0x0001000; // Row Scroll
	PGMPalRAM = (UINT16 *)Next;
	Next += 0x0001400; // Palette R5G5B5
	PGMVidReg = (UINT16 *)Next;
	Next += 0x0010000; // Video Regs inc. Zoom Table

	PGMSprBuf = (UINT16 *)Next;
	Next += 0x0000a00;

	RamEnd = Next;

	RamCurPal = (UINT16 *)Next;
	Next += (0x0001204 / 2) * sizeof(UINT16);
...
static INT32 pgmGetRoms(bool bLoad)
{
	...
	for (INT32 i = 0; !BurnDrvGetRomName(&pRomName, i, 0); i++)
	{
		...
		if ((ri.nType & BRF_GRA) && (ri.nType & 0x0f) == 4)
		{
			if (bLoad)
			{
				...
				{
					if ((PGMSPRMaskROMLoad - PGMSPRMaskROM) == 0x1000000)
	...
	if (!bLoad)
	{
		...
		if (nPGMTileROMLen < 0x400000)
			nPGMTileROMLen = 0x400000;
		...
		if (nPGMARMROMLen == 0)
			nPGMARMROMLen = 0x4000;

		if (nPGMExternalARMLen == 0)
			nPGMExternalARMLen = 0x200000;
	}
...
INT32 pgmInit()
{
	...
	if (BurnLoadRom(PGMTileROM, 0x80, 1))
		return 1;					 // Bios Text and Tiles
...

	{
		...
		for (INT32 i = 0; i < 0x100000; i += 0x08000)
		{ // Video Ram + Mirrors...
...
INT32 pgmFrameBase()
{
	...
	// compile inputs
	{
		...
		// clear opposites
		if ((PgmInput[0] & 0x06) == 0x06)
			PgmInput[0] &= 0xf9; // up/down
		if ((PgmInput[0] & 0x18) == 0x18)
			PgmInput[0] &= 0xe7; // left/right
		if ((PgmInput[1] & 0x06) == 0x06)
			PgmInput[1] &= 0xf9;
		if ((PgmInput[1] & 0x18) == 0x18)
			PgmInput[1] &= 0xe7;
		if ((PgmInput[2] & 0x06) == 0x06)
			PgmInput[2] &= 0xf9;
		if ((PgmInput[2] & 0x18) == 0x18)
			PgmInput[2] &= 0xe7;
		if ((PgmInput[3] & 0x06) == 0x06)
			PgmInput[3] &= 0xf9;
		if ((PgmInput[3] & 0x18) == 0x18)
			PgmInput[3] &= 0xe7;
	...
	for (INT32 i = 0; i < PGM_INTER_LEAVE; i++)
	{
		...
		if (nEnableArm7)
		{
			INT32 cycles = SekTotalCycles() - Arm7TotalCycles();
