...
static void copy_sprite_priority(INT32 prio)
{
	...
	UINT16 *dst = pBurnDraw16;
	for (INT32 i = 0; i < nScreenWidth * nScreenHeight; i++)
	{
		if (pri[i] == prio) {
			dst[i] = RamCurPal[src[i]];
		}
	}
}
...
static void draw_text()
{
	...
	for (INT32 offs = 0; offs < 64 * 32; offs++)
	{
		...
		UINT16 *pal = RamCurPal + color * 0x10;
		UINT16 *dst = pBurnDraw16 + (sy * nScreenWidth) + sx;
...
static void draw_background()
{
	...
	if (t == 0)
	{
		...
		for (INT32 offs = 0; offs < 64 * 16; offs++)
		{
			...
			UINT16 *pal = RamCurPal + color * 0x20;
			UINT16 *dst = pBurnDraw16 + (sy * nScreenWidth) + sx;
	...
	for (INT32 y = 0; y < 224; y++)
	{
		...
		for (INT32 x = 0; x < 480; x+=32)
		{
			...
			UINT16 *dst = pBurnDraw16 + (y * nScreenWidth);
			UINT16 *pal = RamCurPal + color;
