...
// Cond wait loop thread
struct BurnWCLThread
{
    TLOCK Lock;
    TCOND Cond;
    TTHREAD Thd;
...
extern INT16 *pBurnPlaySoundOut;

extern bool bBurnDraw;
extern UINT8 *pBurnDrawBuffers[BURN_MAX_VID_BUFFER];
extern INT32 nBurnDrawBuffersPos;
extern UINT8 *pBurnPlayDrawOut;

extern BurnWCLThread *pBurnDrvDrawThread;
extern BurnWCLThread *pBurnAudPlayThread;
extern BurnWCLThread *pBurnVidPlayThread;
...
void BurnSetPlaySoundOut(INT16 *pSound);
void BurnSetPlayDrawOut(UINT8 *pDraw);
...
INT32 BurnDrvGetDriverList(struct BurnDriver*** pDrv);
...
void IpsApplyPatches(UINT8* base, char* pszRomName);
